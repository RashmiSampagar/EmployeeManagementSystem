...
class Employee:
    def __init__(self,root):
        ...
        self.root.title('Employee ManagementSystem')
        Ibl_title=Label(self.root,text='EMPLOYEE MANAGEMENT SYSTEM',font=('times new roman',37,'bold'),fg='darkblue',bg='white')
...
#main Frame
        ...
        Main_frame.place(x=10,y=90,width=1500,height=700)

#upper Frame
        upper_frame=LabelFrame(Main_frame,bd=2,relief=RIDGE,bg='white',text='employee information',font=('times new roman',11,'bold'),fg='darkblue')
...
#Labels and Entry fields
        ...
        combo_dep['value']=('select department','HR','software engineer','manager','intern')
        combo_dep.current(0)
...
# Address
        ...
        lbl_adderss=Label(upper_frame, font=('arial', 12, 'bold'), text="Address:", bg="white") 
        lbl_adderss.grid(row=2, column=0,sticky=W, padx=2, pady=7)

        txt_adderss=ttk.Entry (upper_frame,textvariable=self.var_address,width=22, font=('arial', 11,'bold'))
        txt_adderss.grid(row=2,column=1,padx=2, pady=7)

# Married

        lbl_married_status=Label(upper_frame,font=('arial', 12, 'bold'), text="Married Type:",bg='white')
...
# Id Proof
        ...
        com_txt_proof['value']=( "PAN CARD", "ADHAR CARD", "DRIVING LICENCE")
...
#callback and validation
        validate_phone=self.root.register(self.checkcontact)
...
#country
        ...
        lbl_country.grid(row=1,column=4, stick=W, padx=2, pady=7)
...
#down Frame
        ...
        down_frame=LabelFrame(Main_frame,bd= 2,relief=RIDGE,bg='white',text='employee information table',font=('times new roman',11,'bold'),fg='darkblue')
        down_frame.place(x=10,y=280,width=1480,height=550)
# search frame
        search_frame=LabelFrame(down_frame,bd=2, relief=RIDGE, bg='white', text='Search Employee Information',font=('times new roman',11,'bold'),fg='blue')
...
#search
        ...
        btn_search=Button(search_frame, text="Search",command=self.search_data, font=("arial",11,"bold"),width=14,bg="darkblue",fg="white")
...
# Table Frame
        ...
        table_frame.place(x=0, y=60, width=1470, height=330)
        ...
        self.employee_table.heading("married", text="Married Type") 
        ...
        self.employee_table.column("idproofcomb",width=108)
    ...
    def add_data(self):
        ...
        if self.var_dep.get()==""or self.var_email.get()=="":
                ...
                try:
                        conn=mysql.connector.connect(host='localhost',username='root', password='T#9758@qlph',database='employeemanagment')
                ...
                if self.var_dep.get()=="" or self.var_email.get()=="":
                        ...
                        try:
                                ...
                                if update>0:
                                        conn=mysql.connector.connect(host='localhost',username='root', password='T#9758@qlph',database='employeemanagment')
                                        my_cursor=conn.cursor()
                                        my_cursor.execute('update employee1 set Department=%s,Name=%s,Designition=%s,Email=%s,Address=%s,Married_Type=%s,DOB=%s,DOJ=%s,id_proof_type=%s,Gender=%s,Phone=%s,Country=%s,Salary=%s where id_proof=%s',(
                                                                                                                                                                                                                                        self.var_dep.get(),
                                                                                                                                                                                                                                        self.var_name.get(),
                                                                                                                                                                                                                                        self.var_designition.get(),
                                                                                                                                                                                                                                        self.var_email.get(),
...
# search
    ...
    def checkcontact(self,Phone_no):
        if Phone_no.isdigit():
            return True
        if len(str(Phone_no))==0:
            return True
        else:
    ...
    def checkemail(self,Email):
        if len(Email)>7:
            if re.match("^([a-zA-Z0-9_\\-\\.]+)@([a-zA-Z0-9_\\-\\.]+)\\.([a-zA-Z]{2,5})$",Email):
                ...
                messagebox.showwarning('Alert','invalid email enter valid user email(xyz@gmail.com)')
                return False
        else:
